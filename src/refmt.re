...
type printType = Bin | Pretty string | Debug | Dump | RoundPretty | RoundDump;

let (command, grammarFile, input) = switch Sysop.argv {
  | [|_, "pretty", grammarFile, input, output|] => (Pretty output, grammarFile, input)
  | [|_, command, grammarFile, input|] => ((switch command {
    | "bin" => Bin
    | "dump" => Dump
    | "pretty" => Pretty "-"
